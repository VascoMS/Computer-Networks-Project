...
void send_messages(){
  ...
  for(int n = get_first_node(); n <= get_last_node(); n = get_next_node(n)){
    if(get_link_cost(n) == COST_INFINITY || get_current_node() == n) continue;
    memcpy(message.data, current_state->distance_vectors[get_current_node()], sizeof(data_t));
    for(int d = get_first_node(); d <= get_last_node(); d = get_next_node(d)){
      if(get_current_node() != d){
        if(current_state->next_hop[d] == n)
          ((data_t*)message.data)->distance_vector[d] = COST_INFINITY;
      }
    }
...
int bellman_ford_recalc(){
  ...
  for(node_t i = get_first_node(); i <= get_last_node(); i = get_next_node(i)){
    ...
    current_state->next_hop[i] = best_hop;
  }
  return changed;
}
